...
def dn_adj(frase, palavra):
    ...
    nome = get_nucleo(arvore_que_estou)
...
def dn_np(frase, palavra):
    print("executando DN_NP")

    minha_arvore = get_minha_arvore(frase, palavra)
    minha_arvore_nucleo = get_nucleo(minha_arvore)

    arvore_no_nao_terminal_nucleo = get_nucleo_sub_arvores(minha_arvore)

    return build_response(minha_arvore_nucleo, arvore_no_nao_terminal_nucleo)



def dn_pp(frase, palavra):
    print("executando DN_PP")
    minha_arvore = get_minha_arvore(frase, palavra)


    arvore_que_estou = get_arvore_que_estou(frase, palavra)
    arvore_que_estou_nucleo = get_nucleo(arvore_que_estou)
...
def get_nucleo_sub_arvores(arvore):
    arvore_no_nao_terminal = get_minha_arvore(arvore, get_no_nao_terminal(arvore))
    arvore_no_nao_terminal_nucleo = None
    while (arvore_no_nao_terminal):
        arvore_no_nao_terminal_nucleo = get_nucleo(arvore_no_nao_terminal)
        if (arvore_no_nao_terminal_nucleo):
            break

        arvore_no_nao_terminal = get_minha_arvore(arvore_no_nao_terminal, get_no_nao_terminal(arvore_no_nao_terminal))

    return arvore_no_nao_terminal_nucleo


def get_no_nao_terminal(frase):
...
def get_arvore_que_estou(frase, palavra):
    palavra_pai = get_pai(frase, palavra)
    return get_minha_arvore(frase, palavra_pai)

def get_pai(frase, palavra):
    palavras = frase.palavras
    index = palavras.index(palavra)
